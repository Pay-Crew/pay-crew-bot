...
import { Client, GatewayIntentBits, Events, ChannelType, CacheType, ChatInputCommandInteraction } from "discord.js";
...
const insert_cmd = async (interaction: ChatInputCommandInteraction<CacheType>) => {
  // 今までの履歴を持ってくる
...
const delete_cmd = async (interaction: ChatInputCommandInteraction<CacheType>) => {
  // 1. 今までの履歴を持ってくる
  const transactions = JSON.parse(readFileSync(STORAGE_PATH, "utf8"));
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.commandName === "insert") {
    await insert_cmd(interaction);
  } else if (interaction.commandName === "delete") {
    await delete_cmd(interaction);
  }
});
